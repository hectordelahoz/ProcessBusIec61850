...
simsignal_t EtherMACFullDuplex::queueingTimeSignal = registerSignal("queueingTime");
...
void EtherMACFullDuplex::initialize(int stage)
{
    ...
    if (stage == 0)
    {   multiCastMACAddrSubs[0] = resolveMultiMACAddress(par("multiCastGroupAddr0"));
        multiCastMACAddrSubs[1] = resolveMultiMACAddress(par("multiCastGroupAddr1"));
        multiCastMACAddrSubs[2] = resolveMultiMACAddress(par("multiCastGroupAddr2"));
        multiCastMACAddrSubs[3] = resolveMultiMACAddress(par("multiCastGroupAddr3"));
        multiCastMACAddrSubs[4] = resolveMultiMACAddress(par("multiCastGroupAddr4"));
        multiCastMACAddrSubs[5] = resolveMultiMACAddress(par("multiCastGroupAddr5"));
        multiCastMACAddrSubs[6] = resolveMultiMACAddress(par("multiCastGroupAddr6"));
        multiCastMACAddrSubs[7] = resolveMultiMACAddress(par("multiCastGroupAddr7"));

        //multiCastMACAddrGo1 = resolveMultiMACAddress(par("multiCastGroupAddrGo1"));
        //multiCastMACAddrGo2 = resolveMultiMACAddress(par("multiCastGroupAddrGo2"));
        //multiCastMACAddrSv1 = resolveMultiMACAddress(par("multiCastGroupAddrSv1"));
        //multiCastMACAddrSv2 = resolveMultiMACAddress(par("multiCastGroupAddrSv2"));
...
void EtherMACFullDuplex::processMsgFromNetwork(EtherTraffic *msg)
{
    ...
    if (!dropFrameNotForUs(frame))
    {
        ...
        {   if(!promiscuous)
            {
                MACAddress destAddr = frame->getDest();
                if(destAddr.isMulticast() && !destAddr.isBroadcast())
                {   int i;
                    for(i = 0; i<8; i++)
                    {   if((multiCastMACAddrSubs[i].getInt() != 0) && (destAddr.equals(multiCastMACAddrSubs[i])))
                        {   break;
                        }
                    }
                    if(i>7)
                    {   EV << "MultiCast Not For Us" << destAddr.getInt() << endl;
                        numDroppedNotForUs++;
                    ...
                    /*if(
                        ( !multiCastMACAddrGo1.getInt() &&
                          !multiCastMACAddrGo2.getInt() &&
                          !multiCastMACAddrSv1.getInt() &&
                          !multiCastMACAddrSv2.getInt()
                        )

                        ||

                        ( !destAddr.equals(multiCastMACAddrGo1) &&
                          !destAddr.equals(multiCastMACAddrGo2) &&
                          !destAddr.equals(multiCastMACAddrSv1) &&
                          !destAddr.equals(multiCastMACAddrSv2)
                        )
                      )
                    {   EV << "MultiCast Not For Us" << destAddr.getInt() << endl;
                        numDroppedNotForUs++;
...
void EtherMACFullDuplex::processReceivedDataFrame(EtherFrame *frame)
{
    ...
    frame->setByteLength(frame->getByteLength() - PREAMBLE_BYTES - SFD_BYTES);//frame->getFrameByteLength());
...
void EtherMACFullDuplex::beginSendFrames()
{
    ...
    else
    {
        ...
        if (!txQueue.extQueue){
            // Output only for internal queue (we cannot be sure that there
...
MACAddress EtherMACFullDuplex::resolveMultiMACAddress(const char *multiCastAddr)
{
    MACAddress multiCastMACAddress;

    if (multiCastAddr[0])
    {   // try as mac address first, then as a module
        multiCastMACAddress.tryParse(multiCastAddr);
    }
    return multiCastMACAddress;
}
