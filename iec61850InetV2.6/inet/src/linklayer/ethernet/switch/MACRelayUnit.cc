...
void MACRelayUnit::handleAndDispatchFrame(EtherFrame *frame)
{
    ...
    //*********************************
    if (dynamic_cast<Ethernet1QTag *>(frame) != NULL)
    {   stPriTag priTag = ((Ethernet1QTag *)frame)->getTci();
        vid = (unsigned int)priTag.vid;
        EV<<"vid = "<<vid<<"\n";
    }
    //*********************************
    // update address table
    addressTable->updateTableWithAddress(inputport, frame->getSrc(),vid);

    // handle broadcast frames first
    if (frame->getDest().isBroadcast())
    {
        EV << "Broadcasting broadcast frame " << frame << endl;
        broadcastFrame(frame, inputport, vid);
        return;
    }

    // Finds output port of destination address and sends to output port
    // if not found then broadcasts to all other ports instead
    // multicast trafic is treated as broadcast (IGMP Snooping must be implemented)
    int outputport = addressTable->getPortForAddress(frame->getDest(),vid);
...
//Limited broadcast depending on VLAN ID.
void MACRelayUnit::broadcastFrame(EtherFrame *frame, int inputport,int vid)
{
    int ports;
    int* portlist;
    portlist = addressTable->getPortForVid(&ports,vid);
    if((vid != 0) && (ports > 1))
    {   for (int i = 0; i<ports; i++)
        {   if (portlist[i] != inputport)
            {   EV<<"Sending msg to port " << portlist[i] <<"\n";
                send((EtherFrame*)frame->dup(), "ifOut", portlist[i]);
            }
        }
    }
    else//*/
