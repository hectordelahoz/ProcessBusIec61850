...
/*
 * returns a list of ports that belongs to a given VID, also receive a pointer that is filled with the size of the list
 */
int* MACAddressTable::getPortForVid(int *numports, unsigned int vid)
{
    Enter_Method("MACAddressTable::getPortForVid()");
    int i = 0;
    int *portlist;
    AddressTable * table = getTableForVid(vid);
    ...
    *numports=table->size();
    portlist = new int[*numports];

    for (AddressTable::iterator iter = table->begin(); iter != table->end(); iter++)
    {   if(iter->second.vid == vid)
        {   portlist[i] = iter->second.portno;
            EV<<"port = "<< iter->second.portno << "\n";
            i++;
        }
    }
    return portlist;
...
void MACAddressTable::readAddressTable(const char* fileName)
{
    ...
    //  Trunk port must be configured belonging to all VLAN ID
    char *line;
    int lineno = 0;
